    ...
    <div>
      <div id="grid-container">
        <div id="intro">
            ...
            Há duas formas de inserir um marcador:
            <ol>
             <li>1º. Insura uma latitude, longitude e uma descrição. Então pressione o botão de adicionar marcador</li> 
             <li>2º. Clique com o lado direito do mouse no mapa e insira uma descrição quando solicitado</li>
            </ol>
        </div>
        ...
        <div id="description" class ='coord-input'>
          <span class="menu-label">Descrição</span><br />
          <input id="description-input" type="text" /><br />
        </div>
  
        <div id="buttons" class ='coord-input'> 
          <input v-on:click="addPinViaInput" type="button" value="Adicionar Marcador" /> <br><br>
          <input v-on:click="summarizeMarkers" type="button" value="Ver todos os marcadores"/>
        </div>
  ...
  export default {
    ...
    data() {
      return {
        ...
        locations: [
          { coord: { lat: -25.40655388182551, lng: -49.21713757604011 }, name: "Hospital Vita" },
          { coord: { lat: -25.416609576502722, lng: -49.22078560542964 }, name: "Hospital Santa Madalena Sofia" },
          { coord: { lat: -25.481037740847373, lng: -49.295378514218704 }, name: "Hospital do Trabalhador" },
        ],
      };
    },
    methods: {
      ...
      summarizeMarkers() {
        ...
        this.locations.forEach(
          (x) =>
            (text =
              text +
              `<li>Latitude: ${x.coord.lat} Longitude: ${x.coord.lng} Descrição: ${x.name}` +
              "</li>")
        );
        text += "</ol>";
