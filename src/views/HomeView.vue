		...
		<header>
			<nav class="navbar navbar-expand-lg navbar-nav" id="navElement">
				<div id="containerNavItems" class="container-fluid">
					...
					<button id="menuSidebarHome" class="navbar-toggler" v-show="shouldShowMenuSidbar" type="button" data-bs-toggle="offcanvas" data-bs-target="#offcanvasNavbarHome" aria-controls="offcanvasNavbar" aria-expanded="false" aria-label="Toggle navigation" role="button">
						<span class="navbar-toggler-icon"></span>
					</button>
					<div class="offcanvas offcanvas-end" data-bs-target="#offcanvas" data-bs-scroll="true" tabindex="-1" id="offcanvasNavbarHome" aria-labelledby="offcanvasNavbarLabelHome">
						...
						<div id="corpoSidebarHome" class="offcanvas-body">
							...
							<div>
								<div class="div-buttons" v-if="!logado">
									<router-link class="btn btn-primary text-light btn-home" to="/signup" tabindex="0">Cadastre-se</router-link>
									<router-link type="button" class="btn btn-primary text-light btn-home" to="/login" tabindex="0">Log-in</router-link>
								</div>
								<div class="div-buttons" v-else>
									<router-link class="btn btn-primary text-light btn-home" to="/dashboard" tabindex="0">Dashboard</router-link>
...
<script>
  ...
  methods: {
    ...
    showMenuSidebar() {
      if (window.scrollY > 5) {
        this.shouldShowMenuSidbar = false;
      }
      else {
        this.shouldShowMenuSidbar = true;
      }
    },
    async signOut(){
      try{
